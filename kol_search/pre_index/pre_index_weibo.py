...
class HandeData():
    ...
    def fetch_raw_data(self):
        sql = 'select userid,screen_name from weibo_kol_cache;'
    ...
    def fetch_excess_data(self,userid):
        sql = "select verified_reason,description from weibo_user_info where userid='%s' limit 1;"%(userid)
        cur = self.conn.cursor()
        cur.execute(sql)
        data = cur.fetchone()
        return data
    ...
    def fetch_hot_words(self,userid):
        sql = "select status_hot_words from weibo_kol where userid='%s' limit 1;"%(userid)
        cur = self.conn.cursor()
        cur.execute(sql)
        data = cur.fetchone()
        if not data:
